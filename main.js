// ═══════════════════════════════════════════════════════════════
// ZEUS MARIO — ULTIMATE PREMIUM EDITION (FIXED)
// All features, controls moved up, fully playable
// ═══════════════════════════════════════════════════════════════
...
const ti = { left:false, right:false, jump:false, down:false, run:false, jumpTap:false };
['Left','Right','Jump','Down','Run'].forEach(n => {
  const b = document.getElementById('btn'+n);
  if (!b) return;
  const k = n.charAt(0).toLowerCase() + n.slice(1);
  b.addEventListener('touchstart', e => { e.preventDefault(); ti[k] = true; if(k==='jump') ti.jumpTap=true; });
  b.addEventListener('touchend', e => { e.preventDefault(); ti[k] = false; });
  b.addEventListener('touchcancel', e => { ti[k] = false; });
});
...
function drawMario(c, x, y, w, h, fr, fc, pu, inv) {
  ...
  const isFire = pu==='fire', isSuper = pu==='super'||pu==='fire';
  const bH = isSuper ? s-4 : s-2;
  // Legs
  c.fillStyle='#2980b9'; const lo=Math.sin(fr*0.25)*3;
  c.fillRect(6,bH-12,8,8+lo); c.fillRect(18,bH-12,8,8-lo);
  c.fillStyle='#5D4037'; c.fillRect(4,bH-4+lo,10,4); c.fillRect(18,bH-4-lo,10,4);
  // Body
  c.fillStyle = isFire?'#e74c3c':'#e53e30';
  c.fillRect(4,8,24,bH-12);
  ...
  // Fire detail
  if(isFire){ c.fillStyle='#f39c12'; c.fillRect(14,bH-18,4,6);
    c.fillStyle='rgba(243,156,18,0.3)'; c.fillRect(6,bH-16,8,3); c.fillRect(18,bH-14,8,3); }
  c.restore();
}
function drawGoomba(c,x,y,w,h,fr,sq){
  c.save(); const s=Math.min(w,h); c.translate(x+w/2-s/2,y+h/2-s/2);
  if(sq){ c.fillStyle='#8B4513'; c.fillRect(0,s-8,s,8);
    c.fillStyle='#000'; c.fillRect(4,s-6,4,2); c.fillRect(s-8,s-6,4,2); c.restore(); return; }
...
function drawFireFlower(c,x,y,s,fr){
  ...
  for(let i=0;i<4;i++){ const a=i/4*Math.PI*2+fr*0.02; c.fillStyle=cols[i];
    c.beginPath(); c.arc(Math.cos(a)*5,Math.sin(a)*5-(s-8),5,0,Math.PI*2); c.fill(); }
...
function drawStar(c,x,y,s,fr){
  ...
  for(let i=0;i<5;i++){ const oa=i*2*Math.PI/5-Math.PI/2,ia=oa+Math.PI/5;
    if(i===0) c.moveTo(Math.cos(oa)*s/2,Math.sin(oa)*s/2);
    else c.lineTo(Math.cos(oa)*s/2,Math.sin(oa)*s/2);
    c.lineTo(Math.cos(ia)*s/4,Math.sin(ia)*s/4); }
...
function drawQB(c,x,y,w,h,fr,hit){
  c.save();
  if(!hit){ c.translate(0,Math.sin(fr*0.1)*1.5); c.shadowColor='#f39c12'; c.shadowBlur=8; }
  else c.globalAlpha=0.6;
  c.fillStyle='#f39c12'; c.fillRect(x,y,w,h);
  c.shadowBlur=0; c.strokeStyle='#d68910'; c.lineWidth=2; c.strokeRect(x,y,w,h);
...
function drawPipe(c,x,y,w,h,fr,hasP,emerging){
  c.save();
  ...
  if(hasP){
    const em=emerging?Math.min(1,Math.sin(fr*0.03)*1.5+0.5):0;
    const py=10-em*22;
    if(em>0.1){
      c.fillStyle='#e74c3c'; c.beginPath(); c.arc(w/2,py+10,12,Math.PI,0); c.fill();
      c.fillStyle='#fff'; c.fillRect(w/2-8,py+6,6,6); c.fillRect(w/2+2,py+6,6,6);
      c.fillStyle='#000'; c.fillRect(w/2-6,py+8,3,4); c.fillRect(w/2+4,py+8,3,4);
      c.fillStyle='#000'; c.beginPath(); c.arc(w/2,py+18,4,Math.PI,0); c.fill();
      c.fillStyle='#fff'; for(let i=-3;i<=3;i+=3) c.fillRect(w/2+i-1,py+14,2,3);
    }
  }
  c.restore();
}
...
function drawFireball(c,x,y,r,fr){
  ...
  c.shadowColor='#f39c12'; c.shadowBlur=15;
  ...
  c.shadowBlur=0;
  for(let i=0;i<3;i++){ const a=fr*0.1+i*2, d=r+4+Math.sin(fr*0.15+i)*2;
    c.fillStyle='rgba(243,156,18,'+(0.3+Math.sin(fr*0.2+i)*0.2)+')'; c.beginPath(); c.arc(Math.cos(a)*d,Math.sin(a)*d,2,0,Math.PI*2); c.fill(); }
  c.restore();
}

// ═══════════ PARTICLES ═══════════
function spawnParticles(x,y,color,count=10,speed=5){
  for(let i=0;i<count;i++){ const a=Math.random()*Math.PI*2, v=speed*(0.3+Math.random()*0.7);
    particles.push({x,y,vx:Math.cos(a)*v,vy:Math.sin(a)*v-2,life:1,decay:0.015+Math.random()*0.025,size:2+Math.random()*4,color,gravity:0.2+Math.random()*0.2}); }
}
...
function spawnBlockParticles(x,y,w,h){
  for(let i=0;i<8;i++){ particles.push({x:x+Math.random()*w,y:y+Math.random()*h,vx:(Math.random()-0.5)*8,vy:-Math.random()*10-3,life:1,decay:0.012+Math.random()*0.015,size:6+Math.random()*6,color:'#cd6155',gravity:0.4,rot:Math.random()*360,rotSpeed:(Math.random()-0.5)*10}); }
}
function spawnPowerupParticles(x,y){
  const cols=['#e74c3c','#f39c12','#27ae60','#3498db','#9b59b6','#f1c40f'];
  for(let i=0;i<20;i++){ particles.push({x,y,vx:(Math.random()-0.5)*10,vy:-Math.random()*10-2,life:1,decay:0.015+Math.random()*0.02,size:3+Math.random()*4,color:cols[Math.floor(Math.random()*cols.length)],gravity:0.15}); }
}
function spawnFlagParticles(x,y){
  const cols=['#FFD700','#e74c3c','#27ae60','#3498db','#f39c12','#fff'];
  for(let i=0;i<40;i++){ const a=Math.random()*Math.PI*2, v=2+Math.random()*6;
    particles.push({x,y,vx:Math.cos(a)*v,vy:Math.sin(a)*v-3,life:1,decay:0.008+Math.random()*0.012,size:3+Math.random()*5,color:cols[Math.floor(Math.random()*cols.length)],gravity:0.05}); }
}
...
function drawParticles(c,camX){
  particles.forEach(p=>{ c.save(); c.globalAlpha=Math.max(0,p.life);
    if(p.rot!==undefined){ c.translate(p.x-camX,p.y); c.rotate(p.rot*Math.PI/180); c.fillStyle=p.color; c.fillRect(-p.size/2,-p.size/2,p.size,p.size); }
    else{ c.fillStyle=p.color; c.beginPath(); c.arc(p.x-camX,p.y,p.size*p.life,0,Math.PI*2); c.fill(); } c.restore(); });
  floatingTexts.forEach(ft=>{ c.save(); c.globalAlpha=Math.max(0,ft.life); c.fillStyle=ft.color;
    c.font='bold '+ft.size+'px Arial'; c.textAlign='center'; c.shadowColor='rgba(0,0,0,0.5)'; c.shadowBlur=4; c.fillText(ft.text,ft.x-camX,ft.y); c.restore(); });
  c.globalAlpha=1;
}

// ═══════════ LEVELS ═══════════
const LEVELS = [
  { name:'WORLD 1-1', subtitle:'Green Plains', bgColor:'#6ec6ff', gradientTop:'#5dade2', gradientBot:'#87CEEB',
    groundColor:'#4a7c3f', groundTopColor:'#27ae60', groundY:420, isUnderground:false, isLava:false, hasWeather:false, gravity:0.55,
    platforms:[
      {x:250,y:350,w:100,h:22,type:'brick'},{x:400,y:310,w:100,h:22,type:'brick'},
      {x:550,y:270,w:80,h:22,type:'question',hasPowerup:'mushroom'},{x:700,y:320,w:120,h:22,type:'brick'},
      {x:900,y:280,w:100,h:22,type:'question',hasPowerup:'coin'},{x:1050,y:340,w:80,h:22,type:'brick'},
      {x:1200,y:290,w:100,h:22,type:'brick'},{x:1400,y:320,w:80,h:22,type:'question',hasPowerup:'mushroom'},
      {x:1550,y:270,w:120,h:22,type:'brick'},{x:1750,y:310,w:100,h:22,type:'brick'},
      {x:2000,y:350,w:80,h:22,type:'question',hasPowerup:'coin'},{x:2150,y:290,w:120,h:22,type:'brick'},{x:2350,y:330,w:100,h:22,type:'brick'}],
  ...
  { name:'WORLD 1-2', subtitle:'Underground Caverns', bgColor:'#0a0a1a', gradientTop:'#1a1a2e', gradientBot:'#0a0a1a',
    groundColor:'#555', groundTopColor:'#666', groundY:420, isUnderground:true, isLava:false, hasWeather:false, gravity:0.58,
    platforms:[
      {x:200,y:360,w:100,h:22,type:'brick'},{x:380,y:320,w:80,h:22,type:'question',hasPowerup:'mushroom'},
      {x:520,y:280,w:100,h:22,type:'brick'},{x:700,y:340,w:80,h:22,type:'question',hasPowerup:'coin'},
      {x:850,y:290,w:120,h:22,type:'brick'},{x:1050,y:330,w:100,h:22,type:'brick'},
      {x:1250,y:280,w:80,h:22,type:'question',hasPowerup:'mushroom'},{x:1400,y:320,w:120,h:22,type:'brick'},
      {x:1600,y:360,w:100,h:22,type:'brick'},{x:1800,y:300,w:80,h:22,type:'question',hasPowerup:'coin'},
      {x:1950,y:270,w:120,h:22,type:'brick'},{x:2150,y:330,w:100,h:22,type:'brick'}],
  ...
  { name:'WORLD 2-1', subtitle:'Sky Fortress', bgColor:'#0a0a3a', gradientTop:'#1a1a4e', gradientBot:'#0a0a2a',
    groundColor:'#5a4a3f', groundTopColor:'#7a5a4f', groundY:440, isUnderground:false, isLava:false, hasWeather:false, gravity:0.52,
    platforms:[
      {x:200,y:370,w:100,h:22,type:'brick'},{x:380,y:330,w:80,h:22,type:'question',hasPowerup:'mushroom'},
      {x:550,y:290,w:120,h:22,type:'brick'},{x:750,y:340,w:80,h:22,type:'question',hasPowerup:'coin'},
      {x:920,y:280,w:100,h:22,type:'brick'},{x:1100,y:320,w:120,h:22,type:'brick'},
      {x:1300,y:270,w:80,h:22,type:'question',hasPowerup:'fire'},{x:1480,y:310,w:100,h:22,type:'brick'},
      {x:1650,y:360,w:120,h:22,type:'brick'},{x:1850,y:300,w:80,h:22,type:'question',hasPowerup:'coin'},
      {x:2000,y:260,w:100,h:22,type:'brick'},{x:2200,y:330,w:120,h:22,type:'brick'}],
  ...
  { name:'WORLD 2-2', subtitle:'Magma Depths', bgColor:'#1a0a0a', gradientTop:'#2a0a0a', gradientBot:'#0a0000',
    groundColor:'#4a2a1a', groundTopColor:'#6a3a2a', groundY:430, isUnderground:false, isLava:true, hasWeather:false, gravity:0.6,
    platforms:[
      {x:200,y:370,w:100,h:22,type:'brick'},{x:400,y:330,w:80,h:22,type:'question',hasPowerup:'mushroom'},
      {x:580,y:290,w:100,h:22,type:'brick'},{x:780,y:340,w:80,h:22,type:'question',hasPowerup:'fire'},
      {x:950,y:290,w:120,h:22,type:'brick'},{x:1200,y:330,w:100,h:22,type:'brick'},
      {x:1400,y:280,w:80,h:22,type:'question',hasPowerup:'coin'},{x:1580,y:320,w:120,h:22,type:'brick'},
      {x:1800,y:370,w:100,h:22,type:'brick'},{x:2000,y:310,w:80,h:22,type:'question',hasPowerup:'mushroom'},
  ...
  { name:'WORLD 3-1', subtitle:'Frozen Tundra', bgColor:'#c8e6ff', gradientTop:'#b3d9ff', gradientBot:'#e0f0ff',
    groundColor:'#b0c4de', groundTopColor:'#d0e4ff', groundY:420, isUnderground:false, isLava:false, hasWeather:true, gravity:0.5,
    platforms:[
      {x:250,y:360,w:100,h:22,type:'brick'},{x:450,y:320,w:80,h:22,type:'question',hasPowerup:'mushroom'},
      {x:620,y:280,w:120,h:22,type:'brick'},{x:820,y:340,w:80,h:22,type:'question',hasPowerup:'star'},
      {x:1000,y:290,w:100,h:22,type:'brick'},{x:1200,y:330,w:120,h:22,type:'brick'},
      {x:1420,y:280,w:80,h:22,type:'question',hasPowerup:'coin'},{x:1600,y:320,w:100,h:22,type:'brick'},
      {x:1800,y:370,w:120,h:22,type:'brick'},{x:2000,y:310,w:80,h:22,type:'question',hasPowerup:'fire'},
  ...
  { name:'WORLD 3-2', subtitle:'Dark Citadel', bgColor:'#0a0000', gradientTop:'#1a0000', gradientBot:'#0a0000',
    groundColor:'#3a2a1a', groundTopColor:'#5a3a2a', groundY:430, isUnderground:true, isLava:true, hasWeather:false, gravity:0.6,
    platforms:[
      {x:200,y:370,w:100,h:22,type:'brick'},{x:400,y:330,w:80,h:22,type:'question',hasPowerup:'fire'},
      {x:600,y:290,w:120,h:22,type:'brick'},{x:800,y:350,w:80,h:22,type:'question',hasPowerup:'star'},
      {x:1000,y:300,w:100,h:22,type:'brick'},{x:1200,y:340,w:120,h:22,type:'brick'},
      {x:1420,y:290,w:80,h:22,type:'question',hasPowerup:'mushroom'},{x:1600,y:330,w:100,h:22,type:'brick'},
      {x:1800,y:370,w:120,h:22,type:'brick'},{x:2000,y:310,w:80,h:22,type:'question',hasPowerup:'coin'},
      {x:2200,y:270,w:120,h:22,type:'brick'},{x:2400,y:320,w:100,h:22,type:'brick'}],
...
function initPlayer(){
  player={x:50,y:350,w:30,h:34,dx:0,dy:0,speed:4.5,runSpeed:6.5,jumpForce:-11,onGround:false,facing:1,frame:0,invincible:0,dead:false,powerUp:null,starTimer:0,isRunning:false,isGroundPounding:false,groundPoundTimer:0,fireCooldown:0};
}
...
function updateGame(){
  ...
  if(toastTimer>0){ toastTimer--; document.getElementById('toast').textContent=toastMsg; document.getElementById('toast').classList.add('show'); }
  ...
  p.isRunning=keys['ShiftLeft']||keys['ShiftRight']||keys['KeyZ']||ti.run;
  ...
  if((keys['ArrowDown']||keys['KeyS']||ti.down)&&!p.onGround&&p.dy>1&&!p.isGroundPounding){ p.isGroundPounding=true; p.groundPoundTimer=10; p.dy=8; }

  if(p.powerUp==='fire'&&p.fireCooldown<=0&&(keys['KeyX']||keys['KeyF']||ti.run)){
    fireballs.push({x:p.x+(p.facing>0?p.w:0),y:p.y+p.h/2,r:6,dx:p.facing*7,dy:-2,frame:0,alive:true,bounce:0});
    p.fireCooldown=15; SFX.fire(); }
  ...
  platforms.forEach(plat=>{
    if(rectCollide(p,plat)){
      if(p.dy>0&&p.y+p.h-p.dy<=plat.y+8){ p.y=plat.y-p.h; p.dy=0; p.onGround=true;
        if(p.isGroundPounding){ p.isGroundPounding=false; screenShake=6;
          if(plat.type==='brick'){ spawnBlockParticles(plat.x,plat.y,plat.w,plat.h); SFX.breakS(); spawnText(plat.x+plat.w/2,plat.y,'SMASH!','#e74c3c',20); plat.type='broken'; plat.w=0; plat.h=0; } } }
      else if(p.dy<0&&p.y-p.dy>=plat.y+plat.h-5){ p.y=plat.y+plat.h; p.dy=0;
        if(plat.type==='question'&&!plat.hit){ plat.hit=true; const pw=plat.hasPowerup||'coin'; spawnPowerup(plat.x+plat.w/2,plat.y-20,pw); SFX.coin(); if(pw==='coin'){score+=10;spawnText(plat.x+plat.w/2,plat.y-30,'+10','#FFD700');} } } }
  });
  ...
  coins.forEach(c=>{
    if(!c.taken){ const dx=(p.x+p.w/2)-c.x, dy=(p.y+p.h/2)-c.y;
      if(Math.sqrt(dx*dx+dy*dy)<22){ c.taken=true; totalCoins++; score+=50; comboCount++; comboTimer=120;
        const bonus=comboCount>1?comboCount*25:0; score+=bonus; SFX.coin(); spawnCoinParticles(c.x,c.y);
        spawnText(c.x,c.y-20,bonus>0?'+'+(50+bonus)+' 🔥':'+50','#FFD700');
        if(comboCount>=5)showToast('🔥 '+comboCount+'x COMBO!'); } }
  });

  enemies.forEach(e=>{
    if(!e.alive)return; e.frame++;
    if(e.inShell){
      if(e.shellMoving){ e.x+=e.dir*6;
        enemies.forEach(o=>{ if(o!==e&&o.alive&&!o.inShell&&rectCollide(e,o)){o.alive=false;spawnStompParticles(o.x+o.w/2,o.y+o.h/2);score+=200;spawnText(o.x,o.y-20,'+200','#ff6b6b');SFX.stomp();} });
        platforms.forEach(plat=>{ if(plat.type==='brick'&&rectCollide(e,plat)){spawnBlockParticles(plat.x,plat.y,plat.w,plat.h);plat.type='broken';plat.w=0;plat.h=0;SFX.breakS();} }); }
      return; }
    ...
    if(rectCollide(p,e)){
      ...
      if(p.dy>0&&p.y+p.h-12<=e.y+8){
        ...
        p.dy=-7; comboCount++; comboTimer=120; const bonus=comboCount>1?comboCount*50:0; score+=100+bonus;
        SFX.stomp(); spawnStompParticles(e.x+e.w/2,e.y+e.h/2); spawnText(e.x,e.y-20,bonus>0?'+'+(100+bonus)+' 🔥':'+100','#ff6b6b');
...
function spawnPowerup(x,y,type){
  if(type==='coin') return;
  powerups.push({x,y,w:20,h:20,type,vy:-3,vx:1,alive:true,frame:0});
  spawnPowerupParticles(x,y); SFX.power();
}

function collectPowerup(type){
  switch(type){
    case 'mushroom': if(!player.powerUp||player.powerUp==='super'){player.powerUp='super';lives++;SFX.oneup();spawnText(player.x,player.y-30,'1-UP! ❤️','#ff6b6b',24);showToast('❤️ 1-UP!');}else{player.powerUp='super';spawnText(player.x,player.y-30,'SUPER!','#e74c3c',22);showToast('🍄 SUPER MARIO!');} break;
...
function hitPlayer(){
  ...
  if(player.powerUp&&player.powerUp!=='star'){player.powerUp=null;player.invincible=90;SFX.hurt();screenShake=5;spawnParticles(player.x+player.w/2,player.y+player.h/2,'#fff',10);spawnText(player.x,player.y-30,'💥 OOF!','#fff',18);showToast('💥 Lost power-up!');}
  else killPlayer();
}
...
function drawBackground(){
  ...
  // Parallax
  drawParallax(0.1,'#fff',0.04,true);
  drawParallax(0.2,level.isUnderground?'#444':'#fff',0.08,level.isUnderground);
  drawParallax(0.4,level.isLava?'#e74c3c':'#fff',0.12,false);
...
function drawParallax(spd,color,alpha,isStar){
  ...
  if(isStar||level.isUnderground) for(let i=0;i<20;i++){ const sx=(i*137.5+off)%(W+100)-50, sy=(i*97.3+30)%(level.groundY*0.6), sz=1+Math.sin(i+frameCount*0.02)*0.5; ctx.fillRect(sx,sy,sz,sz); }
...
function drawGameWorld(){
  ...
  // Pipes
  pipes.forEach(p=>drawPipe(ctx,p.x,gY-p.h,p.width,p.h,frameCount,p.hasPiranha||false,true));
  // Platforms
...
document.getElementById('controlsInfo').style.display='none';
gameLoop();
console.log('🔥 Zeus Mario Ultimate — Ready!');
